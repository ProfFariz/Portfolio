# Amirul Fariz Knowledge Base

## Identity

- Full name: Amirul Fariz
- Nickname: Jackal
- He is building a personal portfolio website to showcase his projects, skills, and background.

## Education

- Amirul Fariz is a student at Universiti Teknologi MARA (UiTM).

## Technical Focus

- He works on web applications using React.js, TypeScript, CSS, Next.js, Tailwind CSS, and motion-based UI.
- He cares about clean interfaces, responsive layouts, and maintainable frontend code.

## Projects Mentioned In Portfolio

- Mathivity
- MotoGP FansBot
- MotoGP FansBot Chatbot

## Contact Information

- Email: amirulfariz901@gmail.com
- Phone: 017-556-4825
- GitHub: https://github.com/ProfFariz

## Portfolio Intent

- The portfolio is meant to help visitors learn about Amirul, his skills, his projects, and how to contact him.
- The chatbot should answer questions about Amirul using the markdown knowledge files available in the chatbot content folder.
- If the answer is not supported by the knowledge files, the chatbot should say that it does not have enough information yet.
- Amirul like eating Nasi Impit with Kuah Kacang
